...
excel_path = get_resource_path("库.xlsx")
color=['r','g','b','c','m','y']
colors = [  
    'red',          # 红色  
    'black',         # 亮绿色  
...
class PSO(AECEnv, EzPickle):
    ...
    def render(self, mode="human"):  
        ...
        if jishu%len(self.agents)==0:
            ...
            for id,agent in enumerate(self.agents):   
                ...
                #     self.main_window.log_to_text_edit(
                #     f"Step: {self.current_step // len(self.agents)}, State: {self.state}, "
                #     f"\nbest: {self.best_scores}, best_position: {self.best_positions}, "
        ...
        if jishu == self.n:
            ...
            plt.plot(liebiao,self.data,color="r")
            plt.show()
            self.data=[]
            jishu =0
            a = self.a
    ...
    def _evaluate(self, position: np.ndarray) -> float:  
        """ 评估适应度 """  
        # params = tuple(np.round(position, 3))  # 保留3位小数精度  
        # return self.param_cache.get(params, -np.inf)  
